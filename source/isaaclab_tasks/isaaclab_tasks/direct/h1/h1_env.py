...
from .h1_env_cfg import H1FlatEnvCfg, H1RoughEnvCfg

class H1Env(DirectRLEnv):
    cfg: H1FlatEnvCfg | H1RoughEnvCfg

    def __init__(self, cfg: H1FlatEnvCfg | H1RoughEnvCfg, render_mode: str | None = None,  context_size = None, **kwargs):
        ...
        self._feet_ids, _ = self._contact_sensor.find_bodies(".*ankle.*")
        self.ankle_ids,_ = self._contact_sensor.find_bodies(".*ankle_link.*")
        self.hip_ids,_ =  self._contact_sensor.find_bodies(".*_(hip_yaw|hip_roll).*")
        self.arm_ids,_ = self._contact_sensor.find_bodies([".*(_shoulder|_elbow).*"])
        ...
        assert False # the ids are wrong (should be joint not body ids)
    ...
    def _setup_scene(self):
        ...
        if isinstance(self.cfg, H1RoughEnvCfg):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        contacts = contact_sensor.data.net_forces_w_history[:, :, self.ankle_ids, :].norm(dim=-1).max(dim=1)[0] > 1.0
        ...
        # compute the reward
        air_time = contact_sensor.data.current_air_time[:, self.ankle_ids]
        contact_time = contact_sensor.data.current_contact_time[:, self.ankle_ids]
        ...
        rewards = {
            ...
            #"lin_vel_z_l2": lin_vel_z_l2 * self.cfg.lin_vel_z_l2_reward_scale * self.step_dt,
        ...
        if self._episode_sums["track_lin_vel_xy_exp"].shape[0] != self.num_envs:
            self._episode_sums = {
            key: torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
            for key in [
                "track_lin_vel_xy_exp",
        ...
        return torch.stack(list(rewards.values())).T#reward
